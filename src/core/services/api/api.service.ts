...
export class ApiService {
  ...
  getLoggedInUserTasks(userId: number): Observable<BaseDataResponse<Tasks[]>> {
    return this.http.get<BaseDataResponse<Tasks[]>>(
      `${this.endpoint}/Tasks/GetByUserId?userId=${userId}`
    ).pipe(
      map((result) => {
        return result;
      })
  ...
  getTaskInfo(): Observable<BaseDataResponse<Tasks>> {
    return this.http
      .get<BaseDataResponse<Tasks>>(this.endpoint + '/Auth/GetTaskInfo')
      .pipe(
        map((result) => {
          return result;
        })
